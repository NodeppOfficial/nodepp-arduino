...
inline ptr_t<uint32> utf8_to_utf32( const ptr_t<uint8>& utf8 ) {
    queue_t<uint32> utf32; ulong i=0; while ( i<utf8.size() ) {
        ...
        if (byte < 0x80) {
            ...
            if (i + 1 >= utf8.size()){ ARDUINO_ERROR( MEMSTR( "Invalid UTF-8 sequence" ) ); return nullptr; }
            codepoint = (byte & 0x1F) << 6 | (utf8[i + 1] & 0x3F);
            i += 2;
        } elif ((byte & 0xF0) == 0xE0) {
            if (i + 2 >= utf8.size()){ ARDUINO_ERROR( MEMSTR( "Invalid UTF-8 sequence" ) ); return nullptr; }
            codepoint = (byte & 0x0F) << 12 | (utf8[i + 1] & 0x3F) << 6 | (utf8[i + 2] & 0x3F);
            i += 3;
        } elif ((byte & 0xF8) == 0xF0) {
            if (i + 3 >= utf8.size()){ ARDUINO_ERROR( MEMSTR( "Invalid UTF-8 sequence" ) ); return nullptr; }
            codepoint = (byte & 0x07) << 18 | (utf8[i + 1] & 0x3F) << 12 | (utf8[i + 2] & 0x3F) << 6 | (utf8[i + 3] & 0x3F);
            i += 4;
        } else { ARDUINO_ERROR( MEMSTR( "Invalid UTF-8 byte" ) ); return nullptr; }
...
inline ptr_t<uint8> utf32_to_utf8( const ptr_t<uint32>& utf32 ) {
    queue_t<uint8> utf8; for ( uint32 codepoint : utf32 ) {
        if (codepoint <= 0x7F) {
            ...
            ARDUINO_ERROR( MEMSTR( "Invalid UTF-32 codepoint" ) ); return nullptr;
        }
    }   return utf8.data();
}
...
inline ptr_t<uint16> utf8_to_utf16( const ptr_t<uint8>& utf8 ) {
    queue_t<uint16> utf16; ulong i = 0; while ( i<utf8.size() ) {
        ...
        if (byte < 0x80) {
            ...
            codepoint = (byte & 0x07) << 18 | (utf8[i + 1] & 0x3F) << 12 | (utf8[i + 2] & 0x3F) << 6 | (utf8[i + 3] & 0x3F);
            i += 4;
        } else { ARDUINO_ERROR( MEMSTR( "Invalid UTF-8 sequence" ) ); return nullptr; }
        ...
        } else { ARDUINO_ERROR( MEMSTR( "Invalid Unicode codepoint" ) ); return nullptr; }
...
inline ptr_t<uint8> utf16_to_utf8(const ptr_t<uint16>& utf16) {
    queue_t<uint8> utf8; ulong i = 0; while ( i<utf16.size() ) {
        ...
        if (unit < 0xD800 || unit > 0xDFFF) { 
            ...
            if (i + 1 >= utf16.size()){ ARDUINO_ERROR( MEMSTR( "Invalid UTF-16 sequence" ) ); return nullptr; }
            uint16 low = utf16[i + 1];
            if (low < 0xDC00 || low > 0xDFFF){ ARDUINO_ERROR( MEMSTR( "Invalid UTF-16 low surrogate" ) ); return nullptr; }
            codepoint = ((unit - 0xD800) << 10) + (low - 0xDC00) + 0x10000;
            i += 2;
        } else { ARDUINO_ERROR( MEMSTR( "Invalid UTF-16 high surrogate" ) ); return nullptr; }
