...
#define coDelay(VALUE)  do { _time_=process::millis()+VALUE; while( process::millis()<_time_ ){ coTime( _time_-millis() ); } } while(0);
#define coUDelay(VALUE) do { _time_=process::micros()+VALUE; while( process::micros()<_time_ ){ coTime( _time_-millis() ); } } while(0);

/*────────────────────────────────────────────────────────────────────────────*/

#define coTime(VALUE)  do { process::set_timeout(VALUE); /*---*/ } while(0);
#define coGoto(VALUE)  do { coSet( VALUE ); coTime(0); return 1; } while(0);
#define coStay(VALUE)  do { coSet( VALUE ); coTime(0); return 0; } while(0);
#define coNext         do { coTime(0); /*----*/ coYield(_LINE_); } while(0);
#define coYield(VALUE) do { coSet( VALUE ); coTime(0); return 1; } while(0);
