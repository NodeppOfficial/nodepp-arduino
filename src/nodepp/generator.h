...
namespace nodepp { namespace generator { namespace ws {
    ...
    template< class T > bool client( T& cli, string_t url ) { do {
        string_t hsh = encoder::key::generate( MEMSTR("abcdefghiABCDEFGHI0123456789") ,22);
        string_t key = string::format("%s==",hsh.data());

        header_t header ({
            { MEMSTR("Upgrade")              , MEMSTR("websocket") },
            { MEMSTR("Connection")           , MEMSTR("upgrade") },
            { MEMSTR("Sec-Websocket-Version"), MEMSTR("13") }
            { MEMSTR("Sec-Websocket-Key")    , key },
        });

        cli.write_header( MEMSTR("GET"), url::path(url), MEMSTR("HTTP/1.1"), header );
        int c=0; while( (c=cli.read_header())==1 )
        /*---------*/ { process::next(); }

        if( c != 0 ){
            cli.onError.emit( MEMSTR("Could not connect to server") );
            cli.close(); break;
        }

        if( cli.status != 101 ){
            cli.onError.emit(string::format( MEMSTR("Can't connect to WS Server -> status %d") ,cli.status));
            cli.close(); break;
        }

        if( cli.headers.has( MEMSTR("Sec-Websocket-Accept") ) ){

            string_t dta = cli.headers[ MEMSTR("Sec-Websocket-Accept") ];
                auto sha = crypto::hash::SHA1(); sha.update( key + NODEPP_WS_SECRET );
            string_t enc = encoder::base64::get( encoder::buffer::hex2buff(sha.get()) );

            if( dta != enc ){
                cli.onError.emit( MEMSTR("secret key does not match") ); 
                cli.close(); break;
            }   cli.stop (); return true;
